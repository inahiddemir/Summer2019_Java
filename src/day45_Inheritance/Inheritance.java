...
/*
 	...
 	features that have default access modifiers: can only be inherited to the classes in same package
 	
 	PROTECTED access modifier: only visible to the classes that are in the same package. but it can be inheritable to the classes outside the package
 	 
 	Not visible but inheritable
 	
 	
 	One class can extends only one class. But one class can be extended BY multiple classes. Thta means a class can only be sub to one class. But super classes can be the super class
 	of bunch of sub classes.
...
public class Inheritance extends TestBase{
				//sub				//super
	//sub class can inherit all the visible and protected futures from super class
